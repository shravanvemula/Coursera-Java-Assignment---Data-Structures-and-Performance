...
public class WPTree implements WordPath {
	...
	{
		List<String> wordPaths=new LinkedList<String>();
		...
		while(!queue.isEmpty()&&!set.contains(word2)) {
			
			WPTreeNode currNode=queue.remove();
			String currWord=currNode.getWord();
			List<String> currNearByWords=nw.distanceOne(currWord,true);
			
			for(String word:currNearByWords) {
			
				if(!set.contains(word)) {
					
					currNode=currNode.addChild(word);
	...
	public static void main(String args[]) {
		...
		List<String> path=wptree.findPath("spell", "mine");
