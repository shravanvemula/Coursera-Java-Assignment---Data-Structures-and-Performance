...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int i=0;i<=s.length();i++) {
			for(int charCode=(int)'a';charCode<=(int)'z';charCode++) {
				...
				if(i!=s.length()) {
				sb.append(s.substring(0,i)).append((char)charCode).append(s.substring(i));
				}
				else {
					sb.append(s+(char)charCode);
				}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty()&&numSuggestions>0&&loopCount<THRESHOLD) {
			String currWord=queue.remove();
			
			List<String> allWords=distanceOne(currWord,true);
			loopCount+=allWords.size();
